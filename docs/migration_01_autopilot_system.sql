-- ============================================================================
-- MIGRATION 1: Marketing Autopilot System
-- Run this first in Supabase SQL Editor
-- ============================================================================

-- Marketing Autopilot Configuration Table
...
-- Autopilot Weekly Reports Table
...
CREATE POLICY "Users can manage their own autopilot config"
  ON marketing_autopilot_config FOR ALL USING (auth.uid() = user_id);

CREATE POLICY "Users can view their own weekly reports"
  ON autopilot_weekly_reports FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can view their own activity log"
  ON autopilot_activity_log FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can manage their lead inbox"
  ON autopilot_lead_inbox FOR ALL USING (auth.uid() = user_id);

-- Indexes for performance
CREATE INDEX IF NOT EXISTS idx_autopilot_config_user_id ON marketing_autopilot_config(user_id);
CREATE INDEX IF NOT EXISTS idx_autopilot_config_active ON marketing_autopilot_config(user_id, is_active);
CREATE INDEX IF NOT EXISTS idx_autopilot_reports_user_date ON autopilot_weekly_reports(user_id, week_start_date DESC);
CREATE INDEX IF NOT EXISTS idx_autopilot_activity_user_date ON autopilot_activity_log(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_autopilot_leads_user_status ON autopilot_lead_inbox(user_id, status, received_at DESC);

-- Helper function to log autopilot activity
...
BEGIN
  INSERT INTO autopilot_activity_log (
    user_id, autopilot_config_id, activity_type, activity_description,
    entity_type, entity_id, metadata
  ) VALUES (
    p_user_id, p_config_id, p_activity_type, p_description,
...
-- Helper function to get autopilot dashboard stats
...
BEGIN
  ...
  SELECT total_leads_generated, total_spend, roi
  INTO v_this_week_leads, v_this_week_spend, v_this_week_roi
  FROM autopilot_weekly_reports
  WHERE user_id = p_user_id AND week_start_date = v_week_start
  LIMIT 1;

  SELECT COUNT(*) INTO v_total_leads
  FROM autopilot_lead_inbox WHERE user_id = p_user_id;
