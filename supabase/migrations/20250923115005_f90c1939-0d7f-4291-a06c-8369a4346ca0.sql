...
-- 1. Fix generated_content table security
-- Add user ownership column
ALTER TABLE public.generated_content ADD COLUMN user_id UUID;

-- Assign existing content to first available user or set to null for now
...
-- Make user_id non-nullable for future records
...
-- Add user ownership column
ALTER TABLE public.social_engagement ADD COLUMN user_id UUID;
...
ALTER TABLE public.social_engagement 
ALTER COLUMN user_id SET NOT NULL;
...
-- 3. Fix ai_insights table - add user ownership
ALTER TABLE public.ai_insights ADD COLUMN user_id UUID;
...
-- 4. Fix analytics_reports table - add user ownership
ALTER TABLE public.analytics_reports ADD COLUMN user_id UUID;
